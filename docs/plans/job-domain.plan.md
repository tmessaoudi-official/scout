...
- [2026-09-24 14:52] NOTED: the first live pass after `49634f1` read hellowork 39 (as the fixtures) and apec **0**. The scrubber had replaced Apec's per-link `id`/`s` with one constant, so a card's four links became identical and the card pattern was written against that; live, each link has its own slot. Fixed by one placeholder per distinct value (fixture now 221/221 links) and a backreference to the shared `e` token; the raw capture reads 45 offers, no miss, no warning.
- [2026-09-24 11:26] NOTED: four Collective.work alerts created in the developer's session, all daily, all Île-de-France, no contract or pay filter: `scout - Dev IDF` (search `Développeur`), `scout - Lead IDF` (`Tech Lead`), `scout - Architecte IDF` (`Architecte`), `scout - DevOps IDF` (`DevOps`). Collective.work sends them to the account's own address, so the Gmail filter that routes them into `job-watch/portails` is still the developer's to add. The source can't be built until the first real email arrives (n=0). **Both halves refuted 18:28, see below: the label already held the sender's mail.**
- [2026-09-24 18:28] NOTED: the n=0 above was wrong. `ops@collective.work` has sent 24 mails into `job-watch/portails` since 2026-09-10, one offer each: subject `[<first name> x <company>] Nouvelle opportunité`, body `Offre : <title>`, a public `collective.work/jobs/fr/<slug>` link and an app link carrying the offer id. No place, pay, contract or work mode. They predate the four alerts, so they are account-level suggestions. The only other unclaimed sender is `contact@free-work.com`, the newsletter already ruled not an alert.
- [2026-09-24 18:28] AGREED: integrate Collective.work now as job source #5, from the mail alone (no offer-page fetch), before Alcopa.
...
- [2026-09-25 16:24] NOTED: issue #17 (the one UNDETECTED ledger case, `job: a card pattern missing a read group loads`) is fixed in b9a9656 — one test case per `CARD_GROUPS` member, each naming `facts` so the place check cannot refuse it first. That commit's message blames the Collective change; the cause is **49634f1** (HelloWork/Apec), which narrowed `CARD_GROUPS` from `['title', 'facts', 'url']` to `['title', 'url']` and left the only case feeding `~(?<title>.+) (?<url>\S+)~` — refused by the new place check before the loop. `Closes #17` closed the issue on push; the proof is the next green nightly.
- [2026-09-25 17:25] AGREED: the green component stays as ruled (23 terms, 3 groups) and its near-silence is accepted — re-judging all 526 stored offers, no source carries a description and green fired on 8 of 409 matches, all from the title; fetching offer pages to feed it is not pursued.
