...
- [2026-09-24 21:52] MEASURED: the Alcopa ALERT is still refused by rule 2 — ten daily mails (15–24 Sep) carry no closing time, no price, no year, no make and no per-lot link, show 3 cars of 192–303, show the SAME 3 for six days, and report a car's mileage differently from one mail to the next (45450 → 45452), so a content key would re-announce it.
- [2026-09-24 21:52] MEASURED, and it REVERSES the 2026-08-29 verdict: Alcopa's pages are NOT JavaScript shells — `JavaScript requis` is a noscript banner above a server-rendered page. robots.txt refuses only `/*.pdf$` and `/calendrier/`. An ONLINE sale page (`/vente-encheres-en-ligne/<id>`, 8 live in `sitemap.sales.xml`) states `Début : 22 septembre 2026, 19:00` and `Flash : 25 septembre 2026, 15:00` and lists lot cards (make, model, version, fuel, year, km, site, `Enchère courante`, lot link) — 20 of its stated 24 on the page. A LOT page (4 735 in `sitemap.items-fr.xml`) carries the full facts and the current bid but only the sale DAY (`Vente du Lundi 12 octobre 2026`, LIVE).
- [2026-09-24 21:52] AGREED: build Alcopa by POLLING, both the online sales and the LIVE saleroom lots (developer choice over online-only). Rule 2 still holds: a lot whose closing time cannot be read is not notified.
- [2026-09-24 22:01] MEASURED — the polling route, end to end (≈15 requests, the project User-Agent, robots read first): (1) the alert's own `/recherche?categories[]=VP&years[]=2020-2100&price_max=30000&km_max=100000&sites[]=1&sites[]=45&sites[]=21` carries no personal token and returns **249 lots, 20 per page**, paginated by a plain GET `page=N` (page 2 of a sale was fetched: 20 new ids, 0 overlap); (2) each search card states make | model, version, fuel code, `1ère mise : <year>`, km, gearbox, storage site, `Mise à prix` or `Enchère courante` (`--` when none), `Lot n°`, the sale DATE and the lot link `/<type>-occasion/<make>/<slug>-<id>` — but NOT its sale; (3) the closing time lives on the SALE page only: online `Début : 22 septembre 2026, 19:00` / `Flash : 25 septembre 2026, 15:00`; LIVE `Lundi 12 octobre 2026 · Horaires : 09:30 - 18:00` (`/salle-de-vente-encheres/<site>/<id>`, 9 listed by `/salle-de-vente-encheres/ventes-en-direct`; online ones in `sitemap.sales.xml`); (4) the per-host pacer applies once per source fetch, and pages inside a fetch run at the adapter's `rate_limit_ms`, so ~13 search pages cost well under a minute, not the 13 minutes a per-request 60 s gap would. OPEN, and it decides the design: whether a card's (site, date) maps to exactly ONE sale — two sales on one site and day would make the closing time ambiguous, and rule 2 then refuses that lot.
- [2026-09-24 22:28] MEASURED — the join, closing the OPEN above: over the saved-search set (249 read of 249 stated, 13 pages) and 17 sales read WITH the saved filters (each page's stated count equals the lots read), 29 lots sit in TWO sales (an online sale and the Nancy LIVE one), and 15 search lots sit in NO listed sale — lot 1093301 links online sale 12902, which `sitemap.sales.xml` does not list. The sale pages ignore the `sites` filter. So the sale list cannot be enumerated reliably; the LOT PAGE names its sale.
- [2026-09-24 22:28] AGREED (design, no-interrupts): Alcopa is a site-specific car adapter `alcopa` — config cannot express the card → lot → sale join or the two closing-time grammars. Each pass: the saved-search pages (the authoritative set), then the lot page of NOVEL lots only behind `lot_budget_per_pass` (the SitemapVehicleSource novelty model), then each referenced sale's header once per pass. A lot whose closing time cannot be read is warned and NOT returned (rule 2 fail-closed), so it is retried. (A tie-break for a lot naming two sales was drafted and dropped: see the next entry.) The three `instanceof SitemapVehicleSource` special cases move to an `IndexedVehicleSource` interface both implement. ≈15 requests per pass.
- [2026-09-24 22:31] MEASURED: a LIVE lot page links its own sale (1120481 → `/salle-de-vente-encheres/beauvais/13026`), and a lot listed in BOTH an online sale and the Nancy LIVE one (1075146) names only ONE on its page, the current online sale 13004. So the lot page's one sale is the closing time's source; a page naming two different sales is ambiguous and is refused like a missing time. A novel lot whose closing instant is already PAST is warned and not returned. The closing time rides on the listing (`VehicleListing::$closingAt`) and is rendered on the push AND the rollup line; online as the Flash instant, LIVE as the sale window, since a saleroom lot has no per-lot hammer time.
- [2026-09-24 22:31] NOTED: rule 2 has never been mechanically enforced — Agorastore is `enabled: true` and its alert carries no closing time (the 2026-08-28 commit said so). Alcopa is the first car source to carry one. Not fixed tonight; written down.
- [2026-09-24 23:07] AGREED (design, no-interrupts, advisor): the closing is two UTC instants on the listing, `saleOpensAt` and `closingAt`, carried by the snapshot, and rendered by ONE formatter helper on the headline the push and every rollup line share — `vente le 12/10 09:30–18:00` when both fall on one Europe/Paris day, else `clôture 25/09 15:00`. Two stated costs, decided once: (1) a queued lot whose closing passes before the daily rollup is ANNOUNCED with its past time printed, not skipped — skip-and-warn without a mark is the phantom-remainder shape `overflow()` already paid for, and the time is on the line; (2) a lot whose snapshot will not decode is rebuilt from the store's columns, which carry no closing, so it goes out without one — the same fallback's §1 cost in CLAUDE.md, extended rather than patched with a column mid-build. The same-day rule infers the kind from the SHAPE: an online sale opening and closing on one day would render as a window, which is still true of it; the 17 sales crawled tonight have none (every online sale spans 2–4 days).
- [2026-09-24 23:27] MEASURED — every search card carries a per-LOT countdown, `<span class="countdown-time" data-ts="<epoch>">` under `title="Temps restant"` (20/20 cards on the saved search, a LIVE sale and an online sale). ONLINE: `data-ts` equals the sale's Flash on 20/20 cards of sale 12918 (25/09 15:00), and lot 1093301's card says 25/09 15:20 while its lot page names a PREVIOUS, finished sale (`Internet | 23-09-2026 16:00:00 … Vente terminée`), so the countdown is the lot's own CURRENT deadline, not a sale-level figure — and possibly staggered per lot, which the sale header cannot show. LIVE: `data-ts` is 30 min before the saleroom opens (Beauvais 09:30 → 09:00, Paris Sud 10:00 → 09:30), while the site's help page says a ceiling bid can be raised *« jusqu'à 15 min avant le début de la vente »* — so its meaning on a LIVE lot is UNSTATED, and it is not the sale window. The lot page states its sale site and start (`Beauvais | 12-10-2026 09:30:00`) and carries the evidence the classifier needs and the card never will — a `Commentaires` block (*« Véhicule grêlé - lunette arriere cassé »*, *« Carte grise sous 30 jours ouvrés »*) and an `Informations` list (*« Liquide de refroidissement à contrôler »*).
- [2026-09-24 23:27] AGREED (design, no-interrupts, advisor) — supersedes the closing-source half of 22:31: an ONLINE lot's `closingAt` is its card `data-ts` (per lot, site-stated); a LIVE lot's `saleOpensAt`/`closingAt` are its sale page's `Horaires` window, one sale header per pass per sale. The lot page is STILL fetched for every novel lot, because it is where the vehicle classifier's evidence lives — skipping it would switch the excluded-vehicle set off on this source. A card whose `data-ts` is already past is dropped BEFORE any lot fetch, so an ended sale costs nothing.
- [2026-09-24 23:27] OPEN (developer's call): hail damage. Sale 13026 is titled *« Vente spéciale véhicules récents et grêlés »* and lot 1120481's comment says *« Véhicule grêlé »*. `grêlé` is not in the excluded vehicle set, so a hail-damaged car is judged like any other. Reject, penalty or nothing is a product decision; nothing is changed until it is made.
- [2026-09-25 00:38] AGREED (developer, AskUserQuestion): hail is a SCORE PENALTY, never a reject — a lot whose own text says `grêlé` keeps its listing but ranks low, so it mostly lands in the daily rollup rather than as an individual push. Not added to the excluded vehicle set. The size is measured through the shipped scorer, not chosen, and the reason line names it.
- [2026-09-25 01:10] MEASURED: through the shipped scorer, an Alcopa card can score at most **65** — price unknown (10 points) and body unknown (25) on every card, since the card states neither and the lot page's `Carrosserie` is the category `VP`, not a body. 25 real cards: 24, 25, then 22 × 40, then one 65. The car push gate is 73, so as built NO Alcopa lot is ever pushed individually, and a lot first seen the morning it closes would reach the developer in the next day's rollup, after it has closed.
- [2026-09-25 01:10] AGREED (developer, AskUserQuestion): an auction lot whose closing falls BEFORE the next daily rollup is pushed individually whatever its score — otherwise it could only be announced after it had closed; every other auction lot goes to the rollup as its score decides, and the rollup still arrives before it closes. Hail's penalty therefore changes rollup ORDER and never makes a closing-soon lot silent.
...
- [2026-09-26 13:46] AGREED (developer, AskUserQuestion): fix Autohero's renamed-slug redirects. Measured live: three lots answered 301 to the SAME uuid under a renamed model slug (relative Location, same host), and were skipped every pass (213 warnings in 72 h, each a budget slot). `SitemapVehicleSource` now follows ONE hop, only when the target is the same host and `item_url_pattern` reads the same id off it, robots-checked and paced; anything else is warned and skipped as before. The moved page's own `offers.url` still names the old slug, so the listing keeps it; it redirects in a browser. Five ledger cases, each reddening a behavioural test.
- [2026-09-26 18:27] AGREED (developer, AskUserQuestion via /next): close CLAUDE.md's open "a followed redirect is never re-checked against robots" finding. Measured: `CurlHttpClient::sendFollowing()` had 0 callers and 0 tests and was the only code that followed redirects without a robots check; libcurl's `FOLLOWLOCATION` is `false`; the only hop production takes is Autohero's `sameLotTarget()`, robots-checked. Deleted the method, pinned `FOLLOWLOCATION => false` with a loopback wire test (redirect to a dead port, so a sabotaged client never leaves the machine) and a ledger case.
...
- [2026-09-07 13:33] RECORD (**ROUND 11 CLEAN — the counter reaches 2 and row 35 closes**).
