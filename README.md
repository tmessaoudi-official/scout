...
the `/job-mission/<category>/<slug>` path), plus **HelloWork's and Apec's alerts** (HTML only; both
keep the offer id inside a base64url token, which the reader decodes). A Free-Work card names no
company, so an offer on both portals is pushed twice.
